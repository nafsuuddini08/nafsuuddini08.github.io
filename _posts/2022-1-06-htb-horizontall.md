...
So if we want to run the exploit it tells us that we need a jwt from a valid user, in this caso i use burpsuite and i will use the jwt of the admin user. 
...
Once we hace access to the machine, we will spawn a pseudo console with python and we will export two environment variables that is ***xterm*** to able to use command like ***clear*** and the shell ***bash***.
